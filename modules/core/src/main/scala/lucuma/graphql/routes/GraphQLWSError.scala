...
enum GraphQLWSError(val code: Int, val reason: String):
  case InvalidMessage(message: String) extends GraphQLWSError(4400, s"Invalid message received: $message")
  ...
  case TooManyInitializationRequests extends GraphQLWSError(4429, "Too many initialization requests")
